...
describe('Parser', () => {
	...
	it('returns an array of record objects with the names of the bday havers', () => {
		...
		expect(parser.parse($)[8].name).toEqual('Hans von Dohnányi')
	})
	...
	it('returns an array of record objects with the wiki url for each bday haver', () => {
		...
		expect(parser.parse($)[8].wikiUrl).toEqual('https://en.wikipedia.org/wiki/Hans_von_Dohn%C3%A1nyi')
	})
	...
	it('returns an array of record objects with the bday date for each bday haver', () => {
		...
		expect(parser.parse($)[8].bday).toEqual('1902-01-01')
	})
	...
	it('correctly handles names that start with a digit', () => {
		expect(parser.parse($)[6].name).toEqual('645AR')
		expect(parser.parse($)[6].wikiUrl).toEqual('https://en.wikipedia.org/wiki/645AR')
		expect(parser.parse($)[6].bday).toEqual('1997-01-01')
		expect(parser.parse($)[6].BCE).toEqual(false)
	})
